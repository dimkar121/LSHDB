            ...
            if (hConf.isKeyed) {
                for (int j = 0; j < keyFieldNames.length; j++) {
                    ...
                    if (keyFieldName.equals(fieldName)) {
                        ...
                        if (!isTokenized) {
                            bfMap.put(keyFieldName, new BloomFilter[]{new BloomFilter(s, ((HammingKey) key).size, 15, 2, true)});
                        } else {
                            String[] os = (String[]) rec.get(keyFieldName + Key.TOKENS);                            
                            BloomFilter[] bfs = new BloomFilter[os.length];
                            for (int k = 0; k < bfs.length; k++) {
                                String v = os[k];
                                bfs[k] = new BloomFilter(v, ((HammingKey) key).size, 15, 2, true);
                            }
